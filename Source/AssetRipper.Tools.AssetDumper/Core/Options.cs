...
public class Options
{
	...
	[Option("validate-comprehensive", Required = false, Default = true,
		HelpText = "Enable comprehensive validation with cross-table checks (requires --validate-schema)")]
	public bool ValidateComprehensive { get; set; } = true;

	[Option("continue-on-error", Required = false, Default = false,
		HelpText = "Continue export on validation errors (otherwise fail-fast)")]
	public bool ContinueOnValidationError { get; set; } = false;

	[Option("max-validation-errors", Required = false, Default = 0,
		HelpText = "Maximum validation errors before stopping (0 = unlimited)")]
	public int MaxValidationErrors { get; set; } = 0;

	[Option("validation-report-path", Required = false, Default = null,
		HelpText = "Custom path for validation report JSON file (default: <output>/validation_report.json)")]
	...
	public bool ValidateComprehensively => ValidateSchemas && ValidateComprehensive;
	public bool ContinueOnError => ContinueOnValidationError;
	public int MaxErrors => MaxValidationErrors;
	public string ValidationReportFilePath => ValidationReportPath ?? Path.Combine(OutputPath, "validation_report.json");
	...
	public void ApplyPreset(ConfigPreset preset)
	{
		switch (preset)
		{
			...
			case ConfigPreset.Full:
				...
				ValidateComprehensive = true;
				ContinueOnValidationError = false; // Fail on errors for quality assurance
				MaxValidationErrors = 0; // Unlimited - catch all errors
				EnableIndexing = true;
			...
			case ConfigPreset.Debug:
				...
				ValidateSchemas = true;
				ValidateComprehensive = true;
				ContinueOnValidationError = true; // Continue to collect all errors for debugging
				MaxValidationErrors = 1000; // Reasonable limit for debugging
				EnableIndexing = true;
				IncludeExtendedMetadata = true;
				TimeoutSeconds = 120;
				break;
