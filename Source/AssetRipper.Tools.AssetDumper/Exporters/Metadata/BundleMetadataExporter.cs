...
/// </summary>
internal sealed class BundleMetadataExporter
{
	...
	public BundleMetadataExporter(Options options, CompressionKind compressionKind, bool enableIndex)
	{
	...
	public DomainExportResult Export(GameData gameData)
	{
		...
		List<BundleMetadataRecord> records = new();
		...
		try
		{
			foreach (BundleMetadataRecord record in records)
			{
	...
	private BundleAggregate TraverseBundle(Bundle bundle, string? parentPk, int depth, List<Bundle> lineage, List<BundleMetadataRecord> records)
	{
		lineage.Add(bundle);
		...
		BundleMetadataRecord record = new()
		{
			Pk = pk,
			Name = bundle.Name,
