...
/// <summary>
/// Tests for DependencyStatsCollector which gathers dependency graph statistics.
/// </summary>
public class DependencyStatsCollectorTests
{
	private readonly string _testOutputPath;

	public DependencyStatsCollectorTests()
	{
	...
	public void Constructor_WithValidOptions_ShouldInitialize()
	{
		...
		collector.MetricsId.Should().Be("dependency_stats");
		collector.SchemaUri.Should().NotBeNullOrEmpty();
		collector.SchemaUri.Should().Contain("dependency_stats");
	}
	...
	{
		// Act
		var act = () => new DependencyStatsCollector(null!);
	...
	[Fact]
	public void MetricsId_ShouldReturnDependencyStats()
	{
		// Arrange
		...
		// Assert
		metricsId.Should().Be("dependency_stats");
	}
	...
	public void SchemaUri_ShouldReturnValidUri()
	{
		...
		schemaUri.Should().Contain("dependency_stats");
	...
	public void WriteMetrics_WithNoData_ShouldNotCreateFile()
	{
		...
		// Assert
		var expectedPath = Path.Combine(_testOutputPath, "metrics", "dependency_stats.json");
