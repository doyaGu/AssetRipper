...
/// <summary>
/// Tests for SceneStatsCollector which gathers scene statistics.
/// </summary>
public class SceneStatsCollectorTests
{
	private readonly string _testOutputPath;

	public SceneStatsCollectorTests()
	{
	...
	public void Constructor_WithValidOptions_ShouldInitialize()
	{
		...
		collector.MetricsId.Should().Be("scene_stats");
		collector.SchemaUri.Should().NotBeNullOrEmpty();
		collector.SchemaUri.Should().Contain("scene_stats");
	}
	...
	{
		// Act
		var act = () => new SceneStatsCollector(null!);
	...
	[Fact]
	public void MetricsId_ShouldReturnSceneStats()
	{
		// Arrange
		...
		// Assert
		metricsId.Should().Be("scene_stats");
	}
	...
	public void SchemaUri_ShouldReturnValidUri()
	{
		...
		schemaUri.Should().Contain("scene_stats");
	...
	public void WriteMetrics_WithNoData_ShouldNotCreateFile()
	{
		...
		// Assert
		var expectedPath = Path.Combine(_testOutputPath, "metrics", "scene_stats.json");
